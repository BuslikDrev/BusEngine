# BusEngine [![BusEngine Version](https://img.shields.io/badge/Release-v0.4.2-black.svg?cacheSeconds=31536000)](https://github.com/BuslikDrev/BusEngine) [![YouTube](https://img.shields.io/youtube/views/QmSmi29DWpU?style=social)](https://www.youtube.com/watch?v=QmSmi29DWpU)
...
Для кроссплатформенности: для сборки игры под Android планируется использовать MSBuild 2017 и Xamarin с Android SDK API 21, а для WebGL OpenSilver или аналоги (скорее просто на чистом js напишу, как и сам движок, вся сила в шейдерах, а код C# в Wasm). Лаунчер движка сам скачает и покажет, что устанавливает с вашего позволения.

Для linux, macos, ios и т.д. нет технической возможности (нет оборудования). Окно для десктопа будет OpenTK.Native 3.3 или glFW (если glFW, то мне по сути весь OpenTK не нужен). Решение будет принято после теста рендера и запуска. Если кто-то желает, то делайте адаптацию и присылайте пулл в репозиторий. Ясность по разграничению устройств и версий будет с BusEngine v0.6.0
...
Без текстур, без освещения, без теней, без тумана, без оптимизации (отрисовка всех полигонов динамических кубов). Отрисовка 6 000 000+ треугольных полигонов, 31252 (х16 в geom shader) или 20000 (х25 в geom shader) модели куба в движении, или 500 (х250 в Instanced):
...
- NVidia GeForce GT 1030 2 GB GDDR5 - умножение полигонов c помощью Instanced (x250), 75+ FPS (1280х720 HD, 1920х1080 Full HD, 2560х1440 2K)
...
BusEngine выпускается под лицензией [MIT](https://github.com/BuslikDrev/BusEngine/blob/main/LICENSE) до версии 0.20.0, с версии 0.20.0 будет видно. Коротко говоря, если будете использовать, то указывайте автора и источник (Удалять splash screen запрещено без согласования с BuslikDrev).
...
~~[Newtonsoft.Json](https://github.com/JamesNK/Newtonsoft.Json) 13.0.3~~  ➔ Заменено на кастомный легковесный парсер в BusEngine.Tools.Json (прирост скорости до 30x, без лишних аллокаций памяти).
